  ...
  startListeningToFlow(requestId: string, args: FlowExecutionArgs): () => void {
    ...
    const unsubscribe = flowEvents.onFlowEvent(requestId, (ev: any) => {
      ...
      if (t === 'toolEnd' && nid) {
        ...
        // index.search / indexSearch → vector search results
        ...
        // agentAssessTask / agent_assess_task → task assessment
        if (
        ...
        // knowledgeBaseSearch / knowledge_base_search → KB search results
        if (
        ...
        // knowledgeBaseStore / knowledge_base_store → KB store result
        if (
        ...
        // workspace.jump / workspaceJump → jump to file result
        ...
        // workspace.map / workspaceMap → workspace overview
        ...
        // code.searchAst / searchAst → AST grep results
