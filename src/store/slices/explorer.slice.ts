/**
 * Explorer Slice
 * 
 * Manages file explorer tree state and operations.
 * 
 * Responsibilities:
 * - Track open/closed folders in the tree
 * - Load directory contents
 * - Toggle folder expansion
 * - Open files for viewing
 * - Manage opened file state
 * 
 * Dependencies:
 * - Workspace slice (for workspace root)
 */

import type { StateCreator } from 'zustand'
import type { ExplorerEntry, OpenedFile } from '../types'

// ============================================================================
// Types
// ============================================================================

export interface ExplorerSlice {
  // State
  explorerOpenFolders: Set<string>
  explorerChildrenByDir: Record<string, ExplorerEntry[]>
  openedFile: OpenedFile | null
  
  // Actions
  loadExplorerDir: (dirPath: string) => Promise<void>
  toggleExplorerFolder: (dirPath: string) => Promise<void>
  openFile: (path: string) => Promise<void>
}

// ============================================================================
// Slice Creator
// ============================================================================

export const createExplorerSlice: StateCreator<ExplorerSlice, [], [], ExplorerSlice> = (set, get) => ({
  // State
  explorerOpenFolders: new Set<string>(),
  explorerChildrenByDir: {},
  openedFile: null,
  
  // Actions
  loadExplorerDir: async (dirPath: string) => {
    if (!window.fs) {
      console.warn('[explorer] window.fs not available')
      return
    }
    
    try {
      const res = await window.fs.readDir(dirPath)
      
      if (res?.success && Array.isArray(res.entries)) {
        // Sort entries: directories first, then alphabetically
        const entries = [...res.entries].sort((a: any, b: any) => {
          // Directories come before files
          if (a.isDirectory !== b.isDirectory) {
            return a.isDirectory ? -1 : 1
          }
          // Alphabetical sort (case-insensitive)
          return a.name.localeCompare(b.name, undefined, { sensitivity: 'base' })
        })
        
        // Update the children for this directory
        set((state) => ({
          explorerChildrenByDir: {
            ...state.explorerChildrenByDir,
            [dirPath]: entries,
          },
        }))
        
        console.debug('[explorer] Loaded directory:', dirPath, `(${entries.length} entries)`)
      } else {
        console.warn('[explorer] Failed to load directory:', dirPath, res)
      }
    } catch (e) {
      console.error('[explorer] Failed to load directory:', dirPath, e)
    }
  },
  
  toggleExplorerFolder: async (dirPath: string) => {
    const state = get()
    const isOpen = state.explorerOpenFolders.has(dirPath)
    
    // Load directory contents if not already loaded and we're opening it
    if (!isOpen && !state.explorerChildrenByDir[dirPath]) {
      await state.loadExplorerDir(dirPath)
    }
    
    // Toggle open state
    const next = new Set(state.explorerOpenFolders)
    if (isOpen) {
      next.delete(dirPath)
      console.debug('[explorer] Closed folder:', dirPath)
    } else {
      next.add(dirPath)
      console.debug('[explorer] Opened folder:', dirPath)
    }
    
    set({ explorerOpenFolders: next })
  },
  
  openFile: async (filePath: string) => {
    if (!window.fs) {
      console.warn('[explorer] window.fs not available')
      return
    }
    
    try {
      const res = await window.fs.readFile(filePath)
      
      if (res?.success && res.content) {
        // Extract filename from path
        const name = filePath.split(/[/\\]/).pop() || filePath
        
        // Infer language from file extension
        const ext = name.split('.').pop()?.toLowerCase()
        const languageMap: Record<string, string> = {
          // JavaScript/TypeScript
          ts: 'typescript',
          tsx: 'typescript',
          js: 'javascript',
          jsx: 'javascript',
          mjs: 'javascript',
          cjs: 'javascript',
          
          // Web
          json: 'json',
          css: 'css',
          scss: 'scss',
          sass: 'sass',
          less: 'less',
          html: 'html',
          htm: 'html',
          xml: 'xml',
          svg: 'xml',
          
          // Markdown
          md: 'markdown',
          mdx: 'markdown',
          
          // Python
          py: 'python',
          pyw: 'python',
          
          // Rust
          rs: 'rust',
          
          // Go
          go: 'go',
          
          // Java
          java: 'java',
          
          // C/C++
          c: 'c',
          h: 'c',
          cpp: 'cpp',
          hpp: 'cpp',
          cc: 'cpp',
          cxx: 'cpp',
          
          // C#
          cs: 'csharp',
          
          // PHP
          php: 'php',
          
          // Ruby
          rb: 'ruby',
          
          // Shell
          sh: 'shell',
          bash: 'shell',
          zsh: 'shell',
          
          // Config
          yaml: 'yaml',
          yml: 'yaml',
          toml: 'toml',
          ini: 'ini',
          
          // SQL
          sql: 'sql',
          
          // Other
          txt: 'plaintext',
          log: 'plaintext',
        }
        
        const language = languageMap[ext || ''] || 'plaintext'
        
        set({
          openedFile: {
            path: filePath,
            content: res.content,
            language,
          },
        })
        
        console.debug('[explorer] Opened file:', filePath, `(${language})`)
      } else {
        console.warn('[explorer] Failed to read file:', filePath, res)
      }
    } catch (e) {
      console.error('[explorer] Failed to open file:', filePath, e)
    }
  },
})

