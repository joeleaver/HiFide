...
export const createPlanningSlice: StateCreator<PlanningSlice, [], [], PlanningSlice> = (set, get) => ({
  ...
  setApprovedPlan: (plan: ApprovedPlan | null) => {
    ...
    console.debug('[planning] Approved plan set:', plan ? `${plan.steps?.length || 0} steps` : 'null')
  },
  ...
  saveApprovedPlan: async () => {
    try {
      ...
      console.debug('[planning] Save result:', res)
      return res
    } catch (e) {
      const state = get() as any
      if (state.addDebugLog) {
  ...
  loadApprovedPlan: async () => {
    try {
      ...
      if (res?.ok) {
        ...
        console.debug('[planning] Loaded plan:', (res as any)?.plan ? 'success' : 'null')
      } else {
        const state = get() as any
        if (state.addDebugLog) {
  ...
  executeApprovedPlanFirstStep: async () => {
    ...
    if (!plan || !Array.isArray(plan.steps) || plan.steps.length === 0) {
      ...
      console.warn('[planning] No plan or steps to execute')
      return
    }
    ...
    console.log('[planning] Executing first step:', first.id)
  ...
  executeApprovedPlanAutonomous: async () => {
    ...
    console.log('[planning] Executing autonomous plan:', plan.steps.length, 'steps')
